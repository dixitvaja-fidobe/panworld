...
class RmaSaleLines(models.Model):
    ...
    tax_id = fields.Many2many(
        "account.tax",
        "account_tax_rma_sale_lines_rel",
        "rma_sale_lines_id",
        "account_tax_id",
        string="Taxes"
    )
    ...
    def create(self, vals_list):
        ...
        res = super(RmaSaleLines, self).create(vals_list)
        for record in res:
            if not record.source_location_id or not record.destination_location_id:
                raise ValidationError(
                    _("Please Configure valid source and destination location in your company!")
                )
        return res
    ...
    def _check_refund_quantity(self):
        for line in self:
            if line.order_quantity < line.refund_qty:
                raise ValidationError(
                    "Return Quantity should not be greater than order quantity"
                )
            if line.order_quantity < line.delivered_quantity:
                raise ValidationError(
                    "Delivered quantity should not be greater than order quantity"
                )
