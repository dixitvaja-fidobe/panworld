...
class RmaDirectLines(models.Model):
    _name = "rma.direct.lines"
    _description = "Return Merchandise Authorization Direct Lines"
    ...
    def create(self, vals_list):
        res = super(RmaDirectLines, self).create(vals_list)
        for record in res:
    ...
    def _get_source_location(self):
        return (
            self.env.user.company_id.sup_source_location_id
    ...
    def _get_destination_location(self):
        return (
            self.env.user.company_id.sup_destination_location_id
            or self.env["stock.location"]
        )
    ...
    tax_id = fields.Many2many(
        "account.tax",
        "account_tax_rma_direct_lines_rel",
        "rma_direct_lines_id",
        "account_tax_id",
        string="Taxes"
    )
    ...
    def default_get(self, fields):
        ...
        res.update(
            {
                "source_location_id": self.env.company.sup_source_location_id.id,
                "destination_location_id": self.env.company.sup_destination_location_id.id,
            }
        )
        return res
