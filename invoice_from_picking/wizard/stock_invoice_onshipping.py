...
class stock_invoice_onshipping(models.TransientModel):
    ...
    def open_invoice(self):
        ...
        if company and self.journal_id.company_id.id != company.id:
            raise UserError(_("Select destination journal base on company!"))
        ...
        if inv_type == "out_invoice":
            action = (
                self.env.ref("account.action_move_out_invoice_type").sudo().read()[0]
            )
    ...
    def create_invoice(self):
        picking_obj = self.env["stock.picking"].browse(self.env.context.get("active_ids"))
        ...
        if type == "incoming":
            ...
            inv_type = "out_invoice"
        res = picking_obj.with_company(pick.company_id).with_context(
            date_inv=self.invoice_date, inv_type=inv_type
        ).action_invoice_create(
            journal_id=self.journal_id.id, group=self.group, move_type=inv_type,
        )
