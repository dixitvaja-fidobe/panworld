...
class eq_inventory_valuation_report_inventory_valuation_report(models.AbstractModel):
    ...
    def convert_withtimezone(self, userdate):
        ...
        if timezone:
            ...
            # print("UTC = ",utc," End_dt = ",end_dt)
            # return end_dt.strftime(DEFAULT_SERVER_DATETIME_FORMAT)

            user_tz = self.env.user.tz or pytz.utc
            local = pytz.timezone(user_tz)
            display_date_result = datetime.strftime(pytz.utc.localize(datetime.strptime(str(userdate), DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local),"%d/%m/%Y %H:%M:%S")
    ...
    def _get_beginning_inventory(self, record, product, warehouse, location=None):
        ...
        from_date = self.convert_withtimezone(start_date_time)
        self._cr.execute('''
                        ...
                        FROM
                            ...
                            WHERE m.create_date <  %s AND (m.location_id in %s) AND m.state='done' AND pp.active=True AND pp.id = %s
                            ...
                            LEFT JOIN uom_uom pu ON (pt.uom_id=pu.id)
                            LEFT JOIN uom_uom u ON (m.product_uom=u.id)
                            WHERE m.create_date < %s AND (m.location_dest_id in %s) AND m.state='done' AND pp.active=True AND pp.id = %s
                            GROUP BY  pp.id,pt.uom_id , m.product_uom ,pp.default_code,u.name,m.date
                            ))
                        AS foo
                        GROUP BY id
    ...
    def get_product_sale_qty(self, record, warehouse, product=None, location=None):
        ...
        if product_data:
            locations = location if location else self.get_location(record, warehouse)
            start_date_time = record.start_date_time.strftime("%Y-%m-%d")  + ' 00:00:00'
            end_date_time = record.end_date_time.strftime("%Y-%m-%d")  + ' 23:59:59'
            ...
            self._cr.execute('''
                            ...
                            ''', (tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), tuple(locations), start_date_time, end_date_time, product_data))
    ...
    def _get_stock_move_valuation(self, record, product, warehouse, op_type, location_ids,quantity):
        ...
        value = 0.00
        if op_type == 'beg':
            domain.append(('create_date', '<', record.start_date_time))
        if op_type == 'in':
            domain += [('create_date', '>=', record.start_date_time),('create_date', '<=', end_date_time),
