...
class wizard_inventory_valuation(models.TransientModel):
    ...
    location_ids = fields.Many2many('stock.location', string='Location', domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")
    # start_date = fields.Date(string="Start Dates")
    # end_date = fields.Date(string="End Dates")
    start_date_time = fields.Datetime(string="Start Timing")
    end_date_time= fields.Datetime(string="End Timing")
    ...
    def convert_withtimezone(self, userdate):
        ...
        if timezone:
            ...
            pytz.utc.localize(datetime.strptime(str(userdate), DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local),"%d/%m/%Y %H:%M:%S")
    ...
    def print_report(self):
        ...
        start_date_time_pdf = self.convert_withtimezone(self.start_date_time)
        end_date_time_pdf = self.convert_withtimezone(self.end_date_time)
        ...
        datas = {'form':
                    {
                        ...
                        'start_date_time': start_date_time_pdf,
                        'end_date_time': end_date_time_pdf,
                        'id': self.id,
    ...
    def print_xls_report(self):
        ...
        for warehouse in self.warehouse_ids:
            ...
            worksheet.write(5, 2, 'Start Date Time', header_merge_format)
            worksheet.write(5, 3, 'End Date Time', header_merge_format)

            worksheet.write(6, 0, self.company_id.name, header_data_format)
            worksheet.write(6, 1, warehouse.name, header_data_format)

            start_date_time = self.convert_withtimezone(self.start_date_time)
            end_date_time = self.convert_withtimezone(self.end_date_time)

            worksheet.write(6, 2, str(start_date_time), header_data_format)
            worksheet.write(6, 3, str(end_date_time), header_data_format)
